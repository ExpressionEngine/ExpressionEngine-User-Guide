...
- Fixed a bug (#23509) where previewing a new entry with an empty Grid and Relationshp would trigger PHP errors.
...
- Fixed a bug where importing a Grid field with a Relationship column disregareded the "allow multiple" setting.
...
- Fixed a bug (#23487) where clicking the reoder handle on a new column in the Grid field settings would refresh the page.
...
- Fixed a bug where the new email and password change notification templates in 4.1 were only added to the primary site.
