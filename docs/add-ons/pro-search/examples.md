...
Have you read the docs on Parameters? Good. Are you familiar with the custom Filters? Excellent. Taken a look at the Template Tags? Perfect. Then here are some examples that illustrate how these three are combined.
...
A standard keyword search, with singular/plural matching and search suggestions when no results are found. Collections need to be created, not specifying them will search all collections.
...
Here are three examples on how to use the Categories filter.
...
Below are two lists of categories. You can select multiple categories per list. Entries that have any categories of the first and second group assigned will be shown. For example: (red or green) and (medium or large).
    ...
    {exp:low_search:form query="{segment_3}"}
      {exp:channel:categories category_group="3" style="linear"}
        <label>
          <input type="checkbox" name="category:color[]" value="{category_id}"{if low_search_category:color ~ '/\b'.category_id.'\b/'} checked{/if}>
          {category_name}
        </label>
      {/exp:channel:categories}
      ...
      {exp:channel:categories category_group="4" style="linear"}
        <label>
          <input type="checkbox" name="category:size[]" value="{category_id}"{if low_search_category:size ~ '/\b'.category_id.'\b/'} checked{/if}>
          {category_name}
        </label>
      {/exp:channel:categories}
...
The Distance filter will take the lat/long values in the distance:from field and filter entries based on the distance from that point to the lat/long values that are stored in the fields defined in the distance:to parameter. Entries will be shown where their distance falls below the threshold defined in the distance:radius parameter. Uses Low List to generate the radius options.
...
You can target specific (custom) fields with the Field Search filter.
Single exact match

Below is a list of countries generated by REEgion Select, which stores its data as a 2-letter country code (alpha2). The form below is identical to adding search:people_region="=XX" to the Results tag, where XX is the country code.

    {exp:low_search:form query="{segment_3}"}
      <select name="search:people_region">
        <option value="">--</option>
        {exp:reegion_select:countries}
          <option value="{region_alpha2}"{if region_alpha2 == low_search_search:people_region} selected{/if}>
            {region_name}
          </option>
        {/exp:reegion_select:countries}
      </select>
    ...
    {exp:low_search:results
      ...
      {if no_results}
        <p>
          ...
          {exp:reegion_select:countries show="{low_search_search:people_region}"}
            {region_name}.
          {/exp:reegion_select:countries}
        </p>
      {/if}
...
Building upon the previous example, you can also select multiple countries. This would be identical to adding search:people_region="=XX|YY" to the Results tag, where XX and YY would be the selected country codes.
    ...
    {exp:low_search:form query="{segment_3}"}
      <select name="search:people_region[]" multiple>
        {exp:reegion_select:countries}
          <option value="{region_alpha2}"{if low_search_search:people_region ~ '/\b'.region_alpha2.'\b/'} selected{/if}>
            {region_name}
          </option>
        {/exp:reegion_select:countries}
      </select>
...
Below is a list of radio buttons, one for each letter of the alphabet. Selecting one of these letters will filter the entries by those whose title starts with the given letter, because of the starts_with parameter on the Results tag. Uses Low List to generate the alphabet.

    {exp:low_search:form query="{segment_3}"}
      {exp:low_list:each items="A|Z" as="letter" range="yes"}
        <label>
          <input type="radio" name="search:title" value="{letter}"{if letter == low_search_search:title} checked{/if}>
          {letter}
        </label>
      {/exp:low_list:each}
      <button type="submit">Go</button>
...
Below is a list of checkboxes based on a custom channel field of the Checkboxes type. You can select multiple options from this list. Entries will be shown that have any of the selected options checked. Uses Low Options to generate field options.

Adding contains_words="parameter_name" to the Results tag will ensure that the selected items are not contained within other words, like appending \W to the values. See related EE docs.
      ...
      {exp:low_options:service_options}
        {options}
          <label>
            ...
            {if low_search_search:service_options ~ '/(^|\|)'.option:value.'(\||$)/'} checked{/if}>
            {option:label}
          </label>
        {/options}
      {/exp:low_options:service_options}
...
The Ranges filter allows you to define from and to ranges that target numeric or date fields.

Numeric fields

Below is a select-element with a list of predefined price ranges. Entries will be shown where the field service_price falls within that range. Uses Low List to generate the options.

    {exp:low_search:form query="{segment_3}"}
      <select name="range:service_price">
        <option value="">--</option>
        {exp:low_list:each items="0|15:15 and below;15|25:15 - 25;25|50:25 - 50;50|:50 and up" sep=";"}
          <option value="{key}"{if key == low_search_range:service_price} selected{/if}>
            {val}
          </option>
        {/exp:low_list:each}
      </select>
...
Note: Low Search also supports Low Events and Solspace Calendar, each with their own set of parameters.
...
The Relationships filter lets you filter entries based on their relationship to other entries. It supports both the native Relationships fields and Playa fields.

Find entries with certain children

As per the Pizza Shop example, the form below lists all pizzas (child entries) and finds the shops (parent entries) that offer any of the selected ones. For example, Margherita or Pepperoni.

    {exp:low_search:form query="{segment_3}"}
      {exp:channel:entries channel="pizzas" dynamic="no"}
        <label>
          <input type="checkbox" name="child:specialty_pizzas[]" value="{entry_id}"{if low_search_child:specialty_pizzas ~ '/\b'.entry_id.'\b/'} checked{/if}>
          {title}
        </label>
      {/exp:channel:entries}
      <button type="submit">Go</button>
...
As per the Music Venue example, the form below lists all bands (parent entries) in a drop down element, and finds all musicians (child entries) in the selected band.

    {exp:low_search:form query="{segment_3}"}
      <select name="parent:member">
        <option value="">--</option>
        {exp:channel:entries channel="bands" dynamic="no"}
          <option value="{entry_id}"{if entry_id == low_search_parent:member} selected{/if}>
            {category_name}
          </option>
        {/exp:channel:entries}
      </select>
...
The Tags filter supports both Solspace Tag and DevDemon Tagger. It works just like the Categories filter.
Single list#
...
Below are two lists of tags. You can select multiple tags per list. Entries that have any tags of the first and second group assigned will be shown. For example: (red or green) and (medium or large).
    ...
    {exp:low_search:form result_page="search/two" query="{segment_3}"}
      {exp:tag:cloud groups="1"}
        <label>
          <input type="checkbox" name="tag_id:color[]" value="{tag_id}"{if low_search_tag_id:color ~ '/\b'.tag_id.'\b/'} checked{/if}>
          {tag}
        </label>
      {/exp:tag:cloud}
      ...
      {exp:tag:cloud groups="2"}
        <label>
          <input type="checkbox" name="tag_id:size[]" value="{tag_id}"{if low_search_tag_id:size ~ '/\b'.tag_id.'\b/'} checked{/if}>
          {tag}
        </label>
      {/exp:tag:cloud}
...
In addition to what all the filters bring to the party, you can also filter by native EE parameters, as well as some little extras.
Orderby and sort in one go#

    {exp:low_search:form query="{segment_3}"}
      ...
      <select name="orderby_sort">
        <option value="low_search_score|desc"{if low_search_orderby_sort == 'low_search_score|desc'} selected{/if}>
          Relevance
        </option>
        <option value="date|asc"{if low_search_orderby_sort == 'date|asc'} selected{/if}>
          Publish date
        </option>
        <option value="title|asc"{if low_search_orderby_sort == 'title|asc'} selected{/if}>
          Title
        </option>
      </select>
...
Based on the native year and month parameters. Uses Low List to generate the options.

    {exp:low_search:form query="{segment_3}"}
      <select name="month">
        {exp:low_list:each items="1|12" as="month" range="yes"}
          <option value="{month}"{if low_search_month == month} selected{/if}>
            {month}
          </option>
        {/exp:low_list:each}
      </select>
      <select name="year">
        {exp:low_list:each items="2015|2005" as="year" range="yes"}
          <option value="{year}"{if low_search_year == year} selected{/if}>
            {year}
          </option>
        {/exp:low_list:each}
      </select>
...
### Display dynamic amount of results per page
    ...
    Uses Low List to generate the options.

    {exp:low_search:form query="{segment_3}"}
      ...
      <select name="limit">
        {exp:low_list:each items="5|10|20|50|100" as="limit"}
          <option value="{limit}"{if limit == low_search_limit} selected{/if}>
            {limit}
          </option>
        {/exp:low_list:each}
      </select>
